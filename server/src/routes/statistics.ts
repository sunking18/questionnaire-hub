...
statisticsRouter.get('/:questionnaireId', authenticate, async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const variance = scores.reduce((sum, s) => sum + Math.pow(s - avgScore, 2), 0) / totalResponses;
    ...
    const questionStats = questions.map((q: any) => {
      ...
      vals.forEach(v => freqMap.set(v, (freqMap.get(v) || 0) + 1));
      let mode = 0;
      let maxFreq = 0;
      ...
      vals.forEach(v => { optDist[v.toString()] = (optDist[v.toString()] || 0) + 1; });
      ...
      return {
        ...
        stddev: Math.round(Math.sqrt(vals.reduce((s, v) => s + Math.pow(v - avg, 2), 0) / vals.length) * 100) / 100,
        median,
        mode,
